  ...
  case DISPLAY_TFT_TTGO:
    if (tft) {
      if (!OLED_display_frontpage) {
        ...
        itoa(ThisStation.addr & 0xFFFFFF, buf, 16);
        ...
        tbw = tft->textWidth(OLED_Protocol_ID[ThisStation.protocol]);

        tft->setCursor(tft->width() - tbw - tft->textWidth(" "),
                       tft->height()/6);
        tft->print(OLED_Protocol_ID[ThisStation.protocol]);
  ...
  case DISPLAY_OLED_HELTEC:
    if (u8x8) {
      if (!OLED_display_frontpage) {
        ...
        u8x8->draw2x2String(14, 2, OLED_Protocol_ID[ThisStation.protocol]);
