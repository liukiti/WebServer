...
void NMEA_loop()
{
  ...
  if (settings->nmea_s && ThisStation.pressure_altitude != 0.0 && isTimeToPGRMZ()) {

    int altitude = constrain(
            (int) (ThisStation.pressure_altitude * _GPS_FEET_PER_METER),
            -1000, 60000);
        ...
        if (Container[i].addr && (this_moment - Container[i].timestamp) <= EXPORT_EXPIRATION_TIME) {
          ...
          if (settings->nmea_l) {
            ...
            if (distance < ALARM_ZONE_NONE) {
              ...
              alt_diff = (int) (Container[i].altitude - ThisStation.altitude);

              if (!Container[i].stealth && !ThisStation.stealth) {
                dtostrf(
...
void NMEA_Position()
{
  ...
  if (settings->nmea_g) {
    ...
    tv.tv_sec  = ThisStation.timestamp;
    tv.tv_usec = 0;

    nmeaTimeSet(&info.utc, &info.present, &tv);

    info.latitude = ((int) ThisStation.latitude) * 100.0;
    info.latitude += (ThisStation.latitude - (int) ThisStation.latitude) * 60.0;
    info.longitude = ((int) ThisStation.longitude) * 100.0;
    info.longitude += (ThisStation.longitude - (int) ThisStation.longitude) * 60.0;
    info.speed = ThisStation.speed * _GPS_KMPH_PER_KNOT;
    info.elevation = ThisStation.altitude; /* above MSL */
    info.height = LookupSeparation(ThisStation.latitude, ThisStation.longitude);
    info.track = ThisStation.course;
